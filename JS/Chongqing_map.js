...
const mapPath = map.selectAll("path")
    .data(Chongqing.features) //数据绑定
    .join("path")
    .attr("d", pathGenerator) //绘制path
    .attr("stroke-width", 1);

mapPath.attr("stroke", "#e7cf8d")
    .attr("fill", "#e7a85c")
    .attr('transform', `translate(50,-50)`);

//地图标点
let points = Chongqing.points.features;
...
map.selectAll("circle")
    ...
    .on("mouseover", function (event) {
        d3.select(this).style("fill", "#2d63ff").attr("stroke", "#ffffff").attr("stroke-width", 2)
        //移除图表
...
map.selectAll("circle")
    .on("mouseout", function (event) {
        d3.select(this).style("fill", "#b85041").attr("stroke", "none");
        //鼠标移开重新渲染

        d3.csv("data/chongqingdata_1.csv", d3.autoType).then(function (data) {
            builder2(data, charts);
        });
        d3.csv("data/chongqingdata_2.csv", d3.autoType).then(function (data) {
            builder1(data, charts,"#93613a","#d58d2c",45,-5);
        });
        info.text('')

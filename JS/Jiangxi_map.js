...
const mapPath = map.selectAll("path")
    .data(Jiangxi.features) //数据绑定
    .join("path")
    .attr("d", pathGenerator) //绘制path
    .attr("stroke-width", 1);

mapPath.attr("stroke", "#9bd2e5")
    .attr("fill", "#517a98")
    .attr('transform', `translate(50,-50)`);

//地图标点
let points = Jiangxi.points.features;
...
map.selectAll("circle")
    ...
    .on("mouseover", function (event) {
        d3.select(this).style("fill", "#efbb4b").attr("stroke", "#ffffff").attr("stroke-width", 2)
        //移除图表
...
map.selectAll("circle")
    .on("mouseout", function (event) {
        d3.select(this).style("fill", "#59debb").attr("stroke", "none");
        //鼠标移开重新渲染

        d3.csv("data/jiangxidata_1.csv", d3.autoType).then(function (data) {
            builder2(data, charts);
        });
        d3.csv("data/jiangxidata_2.csv", d3.autoType).then(function (data) {
            builder1(data, charts,"#4b7caf","#6ab6cb",40,-10);
        });
        info.text('')
